    ...
    <div class="col-md-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                ...
                <button name="reject" type="submit" value="" class="btn btn-success m-3 float-right" title="ADD" data-toggle="modal" data-target="#assigndeduction">
                    ADD
                </button>

                <!-- Add Modal -->
                <div class="modal fade" id="assigndeduction" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="btn-group" role="group" aria-label="Basic example">
                                        ...
                                        <button name="reject" type="submit" value="2" class="text-warning mr-2" title="Reject" data-toggle="modal" data-target="#assigndeductionupdate{{$assign_deduction->id}}">
                                        ...
                                        <!-- Send Reply Modal -->
                                        <div class="modal fade" id="assigndeductionupdate{{$assign_deduction->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                            <div class="modal-dialog" role="document">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        ...
                                                        <div class="modal-body">
                                                            <div class="form-group">
                                                                ...
                                                                <select name="employee_id" disabled>
                                                                    ...
                                                                    <option {{ old('employee_id', $user->id)==$assign_deduction->user_id ? "selected" : "" }} value="{{$user->id}}" disabled="disabled">{{$user->first_name}}</option>
                                                                    @endforeach
                                                                </select>
                                                            </div>
                                                            <div class="form-group">
                                                                ...
                                                                <select name="deduction" id="">
                                                                    <option disabled="disabled" selected>Select Deduction</option>
                                                                    @foreach($deductions as $deduction)
                                                                    <option {{ old('employee_id', $deduction->id)==$assign_deduction->deduction_id ? "selected" : "" }} value="{{$deduction->id}}">{{$deduction->name}}</option>
                                                                    @endforeach
                                                                </select>
                                                            </div>
                                                            <div class="form-group">
                                                                <label for="reply_message">Deduction Amount</label>
                                                                <input type="number" class="form-control" name="rate" value="{{old('rate', $assign_deduction->rate)}}" required>
                                                            </div>
                                                        </div>
                                                        <div class="modal-footer">
                                                            <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                                        ...
                                        <button type="button" class="text-danger" onclick="event.preventDefault(); if(confirm('Are you sure to delete ?')){
                                                        document.getElementById('delete-data-{{ $assign_deduction->id }}').submit();}" data-toggle="tooltip" data-placement="top" title="Delete">
                                            <i data-feather="trash"></i>
                                        </button>
                                        <form id="delete-data-{{ $assign_deduction->id }}" action="{{ route('employer.assigndeduction.destroy', $assign_deduction->id ) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                        </form>
